...
export class LedgerStorage {
  ...
  async _reconcile() {
    if (this.storage.getItem(K_QUARANTINE)) {
      try {
        readJson(this.storage, K_QUARANTINE, null);
        this._snapshot = this.readState();
        return;
      } catch {
        // 冻结标记本身损坏：若后缀副本仍在，说明确曾发生冻结，
        // 用可信前缀重建冻结态，绝不自动解冻继续追加。
        let suffix = [];
        try {
    ...
    try {
      ...
      this._guardedSet(K_CHAIN, JSON.stringify([]));
      this._guardedSet(K_SUFFIX, JSON.stringify([]));
      this._guardedSet(K_QUARANTINE, JSON.stringify({
        firstBadSeq: 1, reason: 'storage-corrupt', at: this.now(), trustedHead: null,
      }));
      this._snapshot = this.readState();
      return;
    }
  ...
  _quarantine(chain, result) {
    ...
    const isolation = {
      ...quarantine,
      phase: 'isolating',
      sourceLength: chain.length,
      prefixLength: prefix.length,
      suffixLength: suffix.length,
    };

    this._guardedSet(K_QUARANTINE, JSON.stringify(isolation));
    this._guardedSet(K_CHAIN, JSON.stringify(prefix));
    this._guardedSet(K_SUFFIX, JSON.stringify(suffix));
    this._guardedSet(K_QUARANTINE, JSON.stringify(quarantine));
    this._guardedSet(K_INTENTS, JSON.stringify([]));
  }

  async _detectAndFreeze() {
    // 事件驱动的检测：仅在当前没有持锁者时才尝试获取锁做裁决。
    if (this.storage.getItem(K_QUARANTINE)) {
      this._snapshot = this.readState();
      return;
    }
